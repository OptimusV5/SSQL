...
void SQL::display(vector<vector<int> > vec, vector<string> str)
{
	...
	{
		cout << "|";
		for (int i = 0; i < (2 * strlength - str[k].length() ) / 2; i++)
			cout << " ";
		cout << str[k];
		for (int i = 0; i < (2 * strlength - str[k].length() + 1) / 2; i++)
			cout << " ";
	}
	cout << "|" << endl;
	for (int i = 0; i < length; i++)
	{
		...
		for (int j = 0; j < str.size(); j++)
		{
			int num = vec[j][i];
			cout << "|";
			for (int k = 0; k < (2 * strlength - digits(num)) / 2; k++)
				cout << " ";
			cout << num;
			for (int k = 0; k < (2 * strlength - digits(num) + 1) / 2; k++)
				cout << " ";
		}
		cout << "|" << endl;
	}
